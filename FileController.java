...
public class FileController {
    ...
    public Result upload(MultipartFile file) {  //文件流的形式接收前端发送过来的文件
        ...
        // 提供文件存储的完整的路径
        // 给文件名 加一个唯一的标识
        String fileName = System.currentTimeMillis() + "_" + originalFileName;  // 156723232323_xxx.png
        String realPath = filePath + fileName;  //  完整的文件路径  D:/.../files/156723232323_xxx.png
        try {
        ...
        //返回一个网络连接
        //http://localhost:8080/files/download/xxxx.jpg
    ...
    * wang-editor编辑器文件上传接口
    ...
    public Map<String, Object> wangEditorUpload(MultipartFile file) {  //文件流的形式接收前端发送过来的文件
        String originalFileName = file.getOriginalFilename();  //xxx.png
        if (!FileUtil.isDirectory(filePath)) {  //如果目录不存在 需要先创建目录
            FileUtil.mkdir(filePath);  //  创建一个 files 目录
        }
